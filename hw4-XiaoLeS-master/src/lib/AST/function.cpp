...
PTypeSharedPtr FunctionNode::getType() {
    return return_type;
}
...
const char *FunctionNode::getArgsCString() const {
    args_string = "";
    for (const auto &parameter : parameters) {
        for (const auto &var : parameter->variables()) {
            args_string.append(var->getTypeCString()).append(", ");
        }
    }

    if (!parameters.empty()) {
            // remove the trailing ", "
        args_string.erase(args_string.end() - 2,
                          args_string.end());
    }
